...
def resize_with_pad_to_square(image, scale_factors, out_sz, padding=(0,0,0)):
    ...
    assert int(scale_factors[0] * image.shape[0]) <= out_sz[1], \
           "int(scale_factors[0] * image.shape[0]) (int({} * {}) = {}) is not less than or equal to out_sz[1] ({})".format(
           scale_factors[0], image.shape[0], int(scale_factors[0] * image.shape[0]), out_sz[1])
    assert int(scale_factors[1] * image.shape[1]) <= out_sz[0], \
           "int(scale_factors[1] * image.shape[1]) (int({} * {}) = {}) is not less than or equal to out_sz[0] ({})".format(
           scale_factors[1], image.shape[1], int(scale_factors[1] * image.shape[1]), out_sz[0])
...
def reverse_resize(image, scale_factors, out_sz, padding=(0,0,0)):
    ...
    assert int((1.0 / scale_factors[0]) * image.shape[0]) <= out_sz[1], \
           "int((1.0 / scale_factors[0]) * image.shape[0]) (int((1.0 / {}) * {}) = {}) is not less than or equal to out_sz[1] ({})".format(
           scale_factors[1], image.shape[1], int((1.0 / scale_factors[0]) * image.shape[0]), out_sz[0])
    assert int((1.0 / scale_factors[1]) * image.shape[1]) <= out_sz[0], \
           "int((1.0 / scale_factors[1]) * image.shape[1]) (int((1.0 / {}) * {}) = {}) is not less than or equal to out_sz[0] ({})".format(
           scale_factors[1], image.shape[1], int((1.0 / scale_factors[1]) * image.shape[1]), out_sz[0])
